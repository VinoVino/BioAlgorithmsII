...
def flipgraph(Node1,Node2,graph):
    ...
    print "node1List ", node1List

    node2List=graph[Node2]
    print  "node2List ", node2List
    node2List.remove(Node1)
    print node2List
    node1Leg1= node1List[1]
    node2Leg1= node2List[1]

    print "graph1 Legs ", node1Leg1, node2Leg1
    ...
    node1Leg2= node1List[1]
    node2Leg2= node2List[0]

    graph3[Node1].append(Node2)
    graph3[Node2].append(Node1)

    print "graph2 legs",  node1Leg2, node2Leg2

    #print "swap_node1Leg1 ", node1Leg1
    #print "swap_node2Leg1 ",node2Leg1
    #add leg1 and leg2 to Node2 and Node1 , swap them
    graph3[Node1].append(node2Leg2)
    graph3[Node2].append(node1Leg2)
    #also do the reverse, have the new leg point back to it's new node
    graph3[node2Leg2].append(Node1)
    graph3[node1Leg2].append(Node2)
    ...
    # find the spot to delete
    for i in range (len(node1Side)):
        if node1Side[i]==node1Leg2:
            delnode1_spot=i
    for i in range(len(node2Side)):
        if node2Side[i]==node2Leg2:
            delnode2_spot=i
    del graph3[Node1][delnode1_spot]
    del graph3[Node2][delnode2_spot]
    # now need to delete moved legs and correct themselves to where they point
    toNode1Side=graph3[node1Leg2]
    toNode2Side=graph3[node2Leg2]
    # find the spot to delete
    ...
    for i in range(len(toNode2Side)):
        if toNode2Side[i]==Node2:
            delToNode2Side_spot=i
    del graph3[node1Leg2][delToNode1Side_spot]
    del graph3[node2Leg2][delToNode2Side_spot]
